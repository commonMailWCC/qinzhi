...
/**
 * Author: liwei  Date: 13-6-21, 上午15:10
 */
...
public final class DateTimeUtils {
    ...
    {
       System.out.println(DateTimeUtils.getHoursOfDay(new Date()));
    }
    
    public static List<Object> getHoursOfDay(Date date){
        List<Object> hours = new ArrayList<>();
        Date startDate = DateTimeUtils.getBeginOfDay(date);
        for(int i =0 ;i<24;i++){
            Date now = DateTimeUtils.addHourOfDay(startDate, i);
            Integer hour = DateTimeUtils.getHourOfDay(now);
            if (hour.toString().length()<2)
            {
                hours.add("0"+ hour.toString());
            }else{
                hours.add(hour);
            }
        }
    ...
    public static List<String> getRangeMonths(Date startDate, Date endDate) {
        ...
        List<String> rangeMonths = Lists.newArrayList();
